...
training_set = dataset_train.iloc[:,
               1:2].values  # should give as range if we give [:,1] we just get a vector what we need is numpy array
...
for i in range(60, 1260):  # 60 inputs till the total range of the dataset
    X_train.append(training_set_scaled[i - 60:i,
                   0])  # 60-60 = 0 so 0 to 60 indexes ,0 is for the column(we have only 1 column now :P)
...
dataset_total = pd.concat((dataset_train['Open'], dataset_test['Open']),
                          axis=0);  # 1 for horizontal concatenation & 0 for vertical
